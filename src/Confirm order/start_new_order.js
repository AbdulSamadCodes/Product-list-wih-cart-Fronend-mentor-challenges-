...
function startNewOrder() {
  ...
  document
    .querySelectorAll(".active-cart__btn")
    .forEach((activeCartBtn) => activeCartBtn.remove());
  Array.from(document.querySelector(".product-cart").children)
    .slice(2)
    .forEach((cartItem) => cartItem.remove());
}
...
function handleStartNewOrderBtn() {
  ...
  startNewBtn.addEventListener("click", function () {
    closeDialogBox();
    startNewOrder();
    updatCartCounter(document.querySelectorAll(".active-cart__btn"));
  });
}
